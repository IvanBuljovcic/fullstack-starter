...
function generateComponent(name: string, options: GeneratorOptions): void {
  ...
  if (fs.existsSync(componentDir)) {
    if (!options.force) {
      console.log(
        chalk.yellow(`⚠️  Component "${componentName}" already exists at:`)
      );
      ...
      console.log(chalk.gray('\nOptions:'));
      console.log(
        chalk.gray('  • Use --force to overwrite existing component')
      );
      console.log(chalk.gray('  • Choose a different name'));
      console.log(
        chalk.gray(
          '  • Use --directory to create in a different location within /src'
        )
      );
      process.exit(1);
    } else {
      console.log(
        chalk.yellow(
          `⚠️  Overwriting existing component "${componentName}" at:`
        )
      );
      console.log(chalk.gray(`   ${componentDir}`));
    }
  ...
  console.log(chalk.gray(`\nFiles created:`));
  console.log(
    chalk.green(`  ✓ ${path.relative(process.cwd(), componentPath)}`)
  );
  if (options.styles) {
    console.log(
      chalk.green(
        `  ✓ ${path.relative(
          process.cwd(),
          path.join(componentDir, `${fileName}.module.css`)
        )}`
      )
    );
  }
...
program
  ...
  .description(
    `Generate Next.js components with templates ${chalk.yellowBright(
...
program
  ...
  .option(
    '-c, --client',
    `Generate client component (default: ${chalk.blueBright(
      'server component'
    )})`
  )
  ...
  .option(
    '-p, --props <props>',
    `Component props (format: ${chalk.blueBright('name: string; age: number')})`
  )
  .option(
    '-d, --directory <path>',
    `Target directory relative to ${chalk.yellowBright(
      '/src'
    )} (default: ${chalk.blueBright('components')})`
  )
  .option(
    '-f, --force',
    `Force ${chalk.redBright('overwrite')} existing component`
  )
