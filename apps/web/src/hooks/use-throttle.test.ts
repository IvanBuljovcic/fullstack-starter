...
describe("useThrottle", () => {
	...
	describe("Argument handling", () => {
		it("should pass arguments to callback", () => {
			const callback = vi.fn();
			const { result } = renderHook(() => useThrottle((a: string, b: number) => callback(a, b), 1000));
		...
		it("should pass arguments from latest allowed call", () => {
			const callback = vi.fn();
			const { result } = renderHook(() => useThrottle((value: string) => callback(value), 1000));
	...
	describe("Hook updates", () => {
		it("should update when callback changes", () => {
			...
			const { result, rerender } = renderHook(({ cb }) => useThrottle(cb, 1000), { initialProps: { cb: callback1 } });
		...
		it("should update when limit changes", () => {
			const callback = vi.fn();
			const { result, rerender } = renderHook(({ limit }) => useThrottle(callback, limit), {
