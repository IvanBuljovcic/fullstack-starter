  ...
  describe('useDebounce', () => {
    describe('Basic functionality', () => {
      ...
      it('should handle different data types', () => {
        // Number
        const { result: numberResult, rerender: numberRerenderer } = renderHook(
          ({ value }) => useDebounce(value),
          {
            initialProps: {
              value: 0,
        ...
        // Object
        const { result: objectResult, rerender: objectRerenderer } = renderHook(
          ({ value }) => useDebounce(value),
          {
            initialProps: {
              value: { count: 0 },
        ...
        // Array
        const { result: arrayResult, rerender: arrayRerenderer } = renderHook(
          ({ value }) => useDebounce(value),
          {
            initialProps: {
              value: [1, 2, 3],
  ...
  describe('useDebounceCallback', () => {
    describe('Basic functionality', () => {
      ...
      it('should pass arguments to callback', () => {
        const callback = vi.fn();
        const { result } = renderHook(() =>
          useDebouncedCallback((a: string, b: number) => callback(a, b), 500)
        );
      ...
      it('should use latest arguments when called multiple times', () => {
        const callback = vi.fn();
        const { result } = renderHook(() =>
          useDebouncedCallback((value: string) => callback(value), 500)
        );
