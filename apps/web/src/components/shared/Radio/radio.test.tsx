...
describe("Radio", () => {
	describe("Basic Rendering", () => {
		...
		it("should show error instead of helper text when both are provided", () => {
			render(<Radio error="Invalid" helperText="Helper text" label="Option 1" />);
	...
	describe("Accessibility", () => {
		...
		it("should have aria-describedby pointing to helper text when provided", () => {
			...
			expect(screen.getByText("Select this option")).toHaveAttribute("id", helperId);
		});
	...
	describe("Controlled Component", () => {
		it("should work as controlled component", async () => {
			...
			const TestComponent = () => {
				...
				return <Radio checked={checked} name="test" onChange={(e) => setChecked(e.target.checked)} />;
			};
		...
		it("should respect controlled checked value", () => {
			const { rerender } = render(<Radio checked={false} name="test" onChange={() => {}} />);
			const radio = screen.getByRole("radio");
			expect(radio).not.toBeChecked();
